...
class QuickUnionByRankWithPathCompression : public UnionFind
{
public:
  /**
   ...
   */
  QuickUnionByRankWithPathCompression(int size) : UnionFind(size), rank(size)
  {
...
{
  int numberOfNodes = 9;
  QuickUnionByRankWithPathCompression graph(numberOfNodes);
  graph.unionSet(1, 2);
  graph.unionSet(1, 0);
  graph.unionSet(1, 4);
